...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
    ...
    if (UART_IsConnected()) {
        ...
        if (GPIO_Pin == Button_Signal_Pin) {
            HAL_GPIO_WritePin(Led_Signal_GPIO_Port, Led_Signal_Pin,
                              GPIO_PIN_RESET);  // NOLINT
