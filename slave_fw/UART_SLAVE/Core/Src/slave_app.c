...
void SlaveApp_Process(void) {
    ...
    LED_SelfTest_Update();
    if (LED_IsTesting() == 0 && slave_state == STATE_IDLE && slave_state != STATE_COMMAND_AUTHORIZED) {
...
void SlaveApp_UART1_RxCallback(uint8_t rx_byte) {
    if (ping_rx_index == 0) {
        ...
        if (ping_rx_index == 4) {
            SIL_ProcessPingFrame((uint8_t*)ping_rx_buffer, (uint8_t*)&last_ping_seq, (uint8_t*)&first_ping_received);
            ping_rx_index = 0;
        }
    }
}
...
void SlaveApp_UART2_RxCallback(uint8_t rx_byte) {
    ...
    if (rx_byte >= '1' && rx_byte <= '8') {
        if(LED_IsTesting()) LED_SelfTest_Abort();
